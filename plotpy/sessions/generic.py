...
"""
  ...
  gnuplotpy=True # use script mode
  ...
  def initialize_gnuplot(self):
    ...
    '''
    if plotting.gnuplot_instance is None:
      ...
      try:
        ...
        plotting.gnuplot_instance=subprocess.Popen([program],
                  stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
  ...
  def import_plugins(self):
    ...
    '''
    from plotpy.plugins import all_plugins
    ...
    if os.path.exists(filename): # Test if the file exists
      ...
      for line in input_file_lines:
        ...
        if (dataset==None and len(line.split())>=2):
          ...
          dataset=mds.MeasurementData(columns, [], 0, 1, 2)
...
#        pickling.close()
      datasets=reader.open(filename)[0]
    if datasets is None:
      return []
  ...
  def plot(self, datasets, file_name_prefix, title, names):
    ...
    if len(datasets)>1:
      ...
      add_info=''
    output=plotting.gnuplot_plotpy\
