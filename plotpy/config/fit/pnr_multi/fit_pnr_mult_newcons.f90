program fit_pnr_mult
  !    Fit of the polarized neutron reflectivity with polarization analysis
  !    Data from TREFF
  !    Super-Parratt formalism
  !    Program by Emmanuel Kentzinger with modifications by Artur Glavic
  !    Files:
  !      parameters_module.f90 fit_pnr_mult_newcons.f90 levenberg_rough_newcons_treff.f90
  !      param_mult.f90 polref_sp_rough.f90 read_mrad_sim_rough_treff.f90 refconv_illu.f90 calchi_treff.f90
  use lay_parameters
  ...
  !! Chcek if this file exists, otherwise end the program
  ...
  write(8,*)
  itest=0
  ! Status report of fit for GUI
  open(82,file='status')
  write(82,*) 'iteration: ',iter,' - chi: ',chi, ' - improvements: ', ichi_improve
  close(82)  
