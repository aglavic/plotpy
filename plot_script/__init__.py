...
def initialize_gui_toolkit():
  ...
  gui_main=__import__('plot_script.'+config.gui.toolkit+'gui.main_window' , fromlist=["main_window"])
  if '--help' not in sys.argv and '--debug' not in sys.argv and len(sys.argv)>1:
    dialogs=__import__('plot_script.'+config.gui.toolkit+'gui.dialogs' , fromlist=["dialogs"])
