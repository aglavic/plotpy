...
from plot_script.read_data import dns as read_data
...
'''
  ...
  FULLAUTO=config.ALWAYS_FULLAUTO
  VANADIUM_FILE=config.VANADIUM_FILE # File name of vanadium file to be used for the correction
  ...
  def autosplit_sequences(self):
    ...
    for prefix in self.prefixes:
      ...
      for i, temp in enumerate(temperatures):
        ...
        if temp-tempi>config.FULLAUTO_TEMP_SENSITIVITY:
          tempi=temp
          splits.append(i)
      splits.append(i+1)
  ...
  def get_temp_current_infos(self, options):
    ...
    for file_name in file_list:
      ...
      if (active_number>=options[3][0]) and (active_number<=options[3][1] or options[3][1]==-1):
        ...
        # Currents
        currents.append((
                         round(read_data.read_info(file_handler, 'Flipper_precession'), 2),
                         #round(read_data.dns.read_info(file_handler, 'Flipper_z_compensation'), 2), 
                         round(read_data.read_info(file_handler, 'C_a'), 2),
                         round(read_data.read_info(file_handler, 'C_b'), 2),
                         round(read_data.read_info(file_handler, 'C_c'), 2),
                         round(read_data.read_info(file_handler, 'C_z'), 2)
                         )
                        )
        # Temperature
        temperatures.append(read_data.read_info(file_handler, 'T1'))
  ...
  def create_maps(self, file_name):
    ...
    if 'detector_angles' in scans[0].dns_info:
      ...
      def append_to_map(point):
        return [detector_bank_2T,
                ...
                point[0]*config.DETECTOR_ANGULAR_INCREMENT+config.FIRST_DETECTOR_ANGLE-detector_bank_2T,
                point[0]
    ...
    for dnsmap in self.active_file_data:
      ...
      if self.VANADIUM_FILE:
        vana_data=read_data.read_data(self.VANADIUM_FILE, print_comments=False)
        if vana_data!='NULL':
...
#      af.insert(0, para_2)
  ...
  def read_vana_bg_nicr_files(self):
    ...
    '''
    directorys=config.SETUP_DIRECTORYS
    ...
    for wildcard in config.NICR_FILE_WILDCARDS:
      nicr_files+=sorted(filter(lambda name: os.path.split(name)[1].startswith(wildcard[0]),
                      filter(lambda name: os.path.split(name)[1].endswith(wildcard[1]),
                             file_list)))
    for wildcard in config.BACKGROUND_WILDCARDS:
      bg_files+=sorted(filter(lambda name: os.path.split(name)[1].startswith(wildcard[0]),
                      filter(lambda name: os.path.split(name)[1].endswith(wildcard[1]),
                             file_list)))
    for wildcard in config.VANADIUM_WILDCARDS:
  ...
  def read_vana_bg_nicr_files_d7(self):
    ...
    # read all nicr files
    for file_name in nicr_files:
      datasets=read_data.read_data_d7(file_name)
      for dataset in datasets:
        dataset.name=os.path.split(file_name)[1]
        detector=round(float(dataset.dns_info['detector_bank_2T']), 0)
        flip=round(float(dataset.dns_info['flipper']), 2)
    ...
    for file_name in vana_files:
      if file_name.endswith('.d7'):
        ...
        datasets=[read_data.read_vana_d7(file_name)]
      for dataset in datasets:
        bgs=False
        if vana_data:
          detector=round(float(dataset.dns_info['detector_bank_2T']), 0)
