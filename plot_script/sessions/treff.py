...
from plot_script.read_data import treff as read_data
from plot_script.read_data import treff_addon1
from plot_script.config import user_config
from plot_script.config import treff as config
from plot_script.config import transformations, gnuplot_preferences
from plot_script.config.scattering_length_table import NEUTRON_SCATTERING_LENGTH_DENSITIES
...
'''
    ...
    if self.import_detector_images:
      ...
      for channel_images in detector_images:
        for image in channel_images:
          ...
          i+=1
          if self.rebinning:
            from plot_script.gtkgui.file_actions import rebin_2d
  ...
  def call_fit_program(self, file_ent, force_compile=False):
    ...
    param_code_file=os.path.join(self.TEMP_DIR, config.PROGRAM_PARAMETER_FILE)
    exe=os.path.join(self.TEMP_DIR, 'pnr.o')
    subcode_files=map(lambda name: os.path.join(code_path, name), config.PROGRAM_FILES)
    ...
    if force_compile or (not os.path.exists(exe)) or \
      ...
      param_code=open(os.path.join(code_path, config.PROGRAM_PARAMETER_FILE), 'r').read()
      ...
      for i, file_ in enumerate([param_code_file]+subcode_files):
        ...
        call_params=[fortran_compiler,
                     config.FORTRAN_PRECOMPILE_OPTION, file_,
                     config.FORTRAN_OUTPUT_OPTION,
                     ofile,
                     ]
...
class TreffFitParameters(FitParameters):
  ...
  def read_params_from_X_file(self, name):
    ...
    '''
    import plot_script.sessions.reflectometer

    ### reading X-ray data
    x_ray_fitdata=plot_script.sessions.reflectometer.RefFitParameters()
