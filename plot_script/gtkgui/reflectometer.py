...
class ReflectometerGUI:
  ...
  def combine_th_scans(self, action, window):
    ...
    '''
    created_map=MeasurementData([], [], 2, 3, 5, 4)
    tth=[]
    th=[]
    I=[]
    for dataset in self.active_file_data:
      if "DRIVE='THETA'" in dataset.info:
        ...
        tth.append(two_theta+dataset.data[0].view(numpy.ndarray)*0.)
        th.append(dataset.data[0].view(numpy.ndarray))
        I.append(dataset.data[1].view(numpy.ndarray))
      elif  "SCAN_ANGLE: THETA" in dataset.info:
        two_theta=float(dataset.info.split('START_2THETA:')[1].split("\n")[0])
        tth.append(two_theta+dataset.data[0].view(numpy.ndarray)*0.)
        th.append(dataset.data[0].view(numpy.ndarray))
        I.append(dataset.data[1].view(numpy.ndarray))
    tth=PhysicalProperty('2Θ', '°', numpy.hstack(tth))
    th=PhysicalProperty('Θ', '°', numpy.hstack(th))
